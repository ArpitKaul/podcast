    ...
    <nav className="md:px-8 lg:px-12 px-4 py-2 mt-3 relative">
      <div className="flex items-center justify-between">
        ...
        <img className='w-10 h-10' src={musical} alt="" />
          <Link to="/" className="text-2xl font-bold  mt-1">Podcaster</Link>
        </div>
        <div className="w-2/6 lg:flex items-center justify-center text-xl hidden">
          {navLinks.map((item, i) => (
        ...
        <Link className='px-6 py-2 border border-black rounded-full text-black'>Login</Link>
        <Link className='ms-4 py-2 border border-black rounded-full text-white bg-black w-20 pl-4'>Signup</Link>
        </div>
        <div className='w-4/6 lg:hidden flex items-center justify-end z-[1000]'>
        <button className='text-5xl' onClick={()    =>setMobileNav(!MobileNav)}>
